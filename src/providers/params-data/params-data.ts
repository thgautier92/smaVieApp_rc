...
export class Paramsdata {
  ...
  All docs are stored in assets/data/docs
  ===== */
  loadDocs() {
    if (this.dataDocs) {
    ...
    return new Promise(resolve => {
      this.http.get('assets/data/docs.json')
        .map(res => res.json())
        .subscribe(data => {
  ...
  displayFile(file) {
    if (file !== "") 
     window.open("file://assets/data/docs/" + file, "_system");
  }
  loadRefs(id?) {
    return new Promise(resolve => {
      if (this.refs) {
        ...
        this.http.get('assets/data/refs.json')
          .map(res => res.json())
          .subscribe(refs => {
            this.refs = refs;
            if (id) {
  ...
  loadForm() {
    return new Promise(resolve => {
      if (this.paramsForm) {
        ...
        this.http.get('assets/data/forms.json')
          .map(res => res.json())
